// ─────────────────────────────────────────────────────────
//  STATS  —  tracks attempted, completed, avg wrong/win
// ─────────────────────────────────────────────────────────
...
async function saveGameResult(result) {
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      tx.update(ref, {
        stats: { ...stats, attempted, completed, hintsUsed, totalWrongOnCompletions,
          streak, bestStreak, [modeKey]: modeStats },
  ...
  recent.forEach(g => {
    ...
    row.innerHTML = `
      <div class="rg-left">
        ...
        <div class="rg-meta">${date} · ${g.mode} · ${g.solved}/${g.total}${wrongStr}${hintStr}</div>
      </div>
...
function resetStatsDisplay() { renderStatsModal({}, []); }
